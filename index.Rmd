...
El Instituto Nacional de Estadística del Perú (INEI), organizó muy bien diversos indicadores para su XI Censo de Población y VI Censo de Vivienda [@inei2007] en su portal para el 2007. Si queriamos descargar los indicadores disponibles para ese censo, simplemente se accede a su [página](http://ineidw.inei.gob.pe/ineidw/) y se selecciona todos los distritos (es lo que queremos en este momento): 
...
Al pedir "procesar" aparece este resultado:

![](inei2017c.png)

Antes de descargaroo, busca darle este formato:

![](inei2017d.png)

Luego, descargalo con CSV, abrelo en Excel y veras algo así:

![](inei2017e.png)

Separalo de manera adecuada (aprovecha de eliminar las primeras dos columnas) y guardalo como xlsx.
...
censo2007=import("data2007.xlsx")
head(censo2007)
```

Veamos que tipos de datos tenemos, y los nombres de columnas:
...
library(reshape2)
censo2007_wide=dcast(data=censo2007, formula=Departamento+Provincia+Distrito ~ Descripcion+Clase, value.var="Valor")
...
h1=formula(algunSeguro_conteo_Total~analfabeta15omas_pct_Total)
...
h1control=formula(algunSeguro_conteo_Total~analfabeta15omas_pct_Total + poblacion_conteo_Total )
...
library(ggplot2)
VarDep=censo2007_wide_Totales$algunSeguro_conteo_Total
...
base=ggplot(data=censo2007_wide_Totales, aes(x=algunSeguro_conteo_Total)) + theme_classic()
...
rp1=glm(h1, data = censo2007_wide_Totales, 
        offset=log(poblacion_conteo_Total), #exposure 
...
h2=formula(algunSeguro_conteo_Total~analfabeta15omas_pct_Total + trabIndepNoProf_pct_Total)
    
rp2=glm(h2, data = censo2007_wide_Totales, 
        offset=log(poblacion_conteo_Total),
        family = poisson(link = "log"))
...
La interpretación de los coeficientes de la Tabla \@ref(tab:rp1rp2) requiere cálculos previos:
...
\end{equation}
```
Cuando el *exposure* es constante modelamos conteos (Y); cuando no lo es modelamos ratios ($\lambda$)[^4]. Pero, como vemos en la Ecuación \@ref(eq:poi), los coeficiente necesitan ser exponenciados para saber el efecto sobre Y. Veamos la Tabla \@ref(tab:exp-rp2).

[^4]: Los conteos con *offset*.
...
```{r}
summary(rp2)$dispersion;summary(rqp)$dispersion
```
...
Una buena ante la sobredispersión es usar la *regresión binomial negativa*. Comparemos todas la regresiones exponenciadas, como se ve en la Tabla \@ref(tab:exp-rbn).

```{r exp-rbn,warning=FALSE, message=FALSE, results='asis'}
h2off=formula(algunSeguro_conteo_Total ~ analfabeta15omas_pct_Total + trabIndepNoProf_pct_Total + offset(log(poblacion_conteo_Total)))
...
Nótese en la Tabla \@ref(tab:exp-rbn) que los coeficientes obtenidos en la regresión binomial negativa son diferentes a los demas. Además, los AIC son mucho mejores para caso de la Binomial Negativa (recuerda los AIC de la Tabla \@ref(tab:rp1rl2)).
...
```{r stdCoefs}
sdVD=sd(censo2007_wide_Totales$algunSeguro_conteo_Total)
sdVIs=apply(censo2007_wide_Totales[,c("algunSeguro_conteo_Total","trabIndepNoProf_pct_Total")],2,sd)
